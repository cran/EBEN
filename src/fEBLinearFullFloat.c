//version 3.0 hard version: BASIS_PHI was updated via copying. further development: by change the address pointers pointing to.
//version 3.1 soft version: BASIS_PHI change pointer address
#ifndef  USE_FC_LEN_T
# define USE_FC_LEN_T
#endif
...
//G: gauss; F:full; Neg: normal exp gauss
void LinearSolverGFNeg(double * a, double *logout, int N,int M,double *output);
void fEBInitializationGFNeg(double *Alpha, double * PHI, int *Used, int *Unused,
				double *BASIS, double *Targets, float *Scales, int * initial,
				int n, int *m, int kdim, double *beta);

void MatrixInverseGFNeg(double * a,int N);
void CacheBP(float **BASIS_PHI, float *BASIS_Targets, double *BASIS, double *PHI,
				double *Targets,float *scales,int N,int K,int M, int M_full);

void fEBLinearFullStatGFNeg(double *beta,double * SIGMA,double *H, float *S_in, float * Q_in, float * S_out,
				float * Q_out,  double *BASIS, float * Scales, double *PHI, float **BASIS_PHI,
				float *BASIS_Targets, double * Targets, int * Used, double *Alpha, double * Mu,
				 double *Gamma,int *n, int *m, int* kdim, int *iteration,int *i_iter);

void fEBDeltaMLGFNeg(double *DeltaML, int *Action, double *AlphaRoot, int *anyToDelete,
				int *Used, int * Unused, float * S_out, float * Q_out, double *Alpha,
				double *a_gamma, double *b_gamma, int m, int mBar);

void fEBDeltaMLgroupGFNeg(double *DeltaML, int *Action, double *AlphaRoot, int *anyToDelete,
				int *Used, int * Unused, float * S_out, float * Q_out, double *Alpha,
				double *a_gamma, double *b_gamma, int N_used, int N_unused,int k);

void LinearFastEmpBayesGFNeg(int *Used, double *Mu, double *SIGMA, double *H, double *Alpha, double *PHI,
				...
				int verbose, int group);



int ActionAddGFNeg(float **BASIS_PHI, double* BASIS, float*scales, double*PHI, double*Phi,
			double *beta, double* Alpha, double newAlpha, double*SIGMA, double*Mu, float*S_in,
			float*Q_in, int nu, double*SIGMANEW, int M_full, int N, int K, int M);


int ActionDelGFNeg(double*PHI, double*Alpha, double*SIGMA, double*SIGMANEW, float**BASIS_PHI,
				double*Mu, float*S_in, float*Q_in, double *beta, int jj, int N, int M, int M_full);

double varTargetsGFNeg(double* Target,int N);

void FinalUpdateGFNeg(double *PHI,double * H,double*SIGMA,double *Targets,double *Mu,double *Alpha,
...
//------------------------------------------------------------
	...
	while (iter<iter_max && err>err_max)
	{
		...
		LinearFastEmpBayesGFNeg(Used, Mu, SIGMA, H, Alpha,PHI,	BASIS, Targets,Scales, a_gamma, b_gamma,
						iteration, n, kdim, m,basisMax,&b,&beta,C_inv,verbose,group);
				...
				int verbose, int group)
{
   ...
   int N,K,M_full,N_unused,M,i,j,L,iter,kk;
	...
	fEBInitializationGFNeg(Alpha, PHI, Used, Unused, BASIS, Targets, Scales, IniLogic, N, m, K,beta);
	...
	//BASIS_PHI			= (double *) R_Calloc(M_full*basisMax,double);
	//double scal = 1.0;
	float *BASIS_Targets,**BASIS_PHI;
	//int MMax 			= (M_full*basisMax);
	//Rprintf("M_full: %d; basisMax: %d; maxMemory size: %d\n",M_full, basisMax,MMax);
	BASIS_Targets		= (float *) R_Calloc(M_full,float);
	BASIS_PHI			= (float **) R_Calloc(basisMax,float);
	for(i=0;i<M;i++)
	{
		BASIS_PHI[i] 	= (float *) R_Calloc(M_full,float);
	}
...
//for(i=0;i<100;i++) Rprintf("BASIS_T: %f\n",BASIS_Targets[i]);
    //              For: [DeltaML,Action,AlphaRoot,anyToDelete]     = fEBDeltaMLGFNeg(Used,Unused,S_out,Q_out,Alpha,a,b);
   ...
   while(LAST_ITERATION!=1)
    {
        ...
        //[DeltaML,Action,AlphaRoot,anyToDelete]     = fEBDeltaMLGFNeg(Used,Unused,S_out,Q_out,Alpha,a,b);
		if(group ==0)
		{
			fEBDeltaMLGFNeg(DeltaML, Action, AlphaRoot,anyToDelete,Used, Unused, S_out, Q_out, Alpha,
				a_gamma,b_gamma, M, N_unused);
		}else		//group eblasso
		{
			fEBDeltaMLgroupGFNeg(DeltaML, Action, AlphaRoot,anyToDelete,Used, Unused, S_out, Q_out, Alpha,
				a_gamma,b_gamma, M, N_unused,K);
		}
		//
        ...
        for(i=0;i<M_full;i++)
        {
            if(DeltaML[i]>deltaLogMarginal && DeltaML[i]<1e5)
            {
        ...
        {
            if (fabs(log(newAlpha)-log(Alpha[jj]))<=1e-2 && anyToDelete[0] ==0)
            {
	...
	//Rprintf("\t selectedAction: %d\n",selectedAction);
        ...
        if(selectedAction==ACTION_REESTIMATE)
        {
			...
			{
				temp	= 0;
				//for(j=0;j<M;j++) temp = temp + (double)BASIS_PHI[j*M_full + i]*SIGMA[jj*M + j];
				for(j=0;j<M;j++) temp = temp + (double)BASIS_PHI[j][i]*SIGMA[jj*M + j];
				//readPtr1 			= &BASIS_PHI[i];
				//readPtr2 			= &SIGMA[jj*M];
				//temp = F77_CALL(ddot)(&M, readPtr1, &M_full,readPtr2, &inci);
				S_in[i]				= S_in[i] + (float)(pow(beta[0]*temp,2)*kappa);
				Q_in[i]				= Q_in[i] +  (float)(beta[0]*Mujj *kappa*temp);
			}
			//
			//for(i=0;i<M_full;i++)
			//{
        ...
        else if(selectedAction==ACTION_ADD)
        {
			...
			//allocate one more row of BASIS_PHI
			//BASIS_PHI[M] 	= (float *) R_Calloc(M_full,float);
			UPDATE_REQUIRED		= ActionAddGFNeg(BASIS_PHI, BASIS, Scales, PHI, phi, beta, Alpha,
        ...
        else if(selectedAction==ACTION_DELETE)
        {
            ...
            UPDATE_REQUIRED = ActionDelGFNeg(PHI, Alpha, SIGMA, SIGMANEW, BASIS_PHI,
		...
		if(UPDATE_REQUIRED==1)
        {
		    ...
		    {
				index					= Used[i] -1;
				S_out[index]			= (float)(Alpha[i]*(double)S_in[index]/(Alpha[i]-(double)S_in[index]));
				Q_out[index]			= (float)(Alpha[i]*(double)Q_in[index]/(Alpha[i]-(double)S_in[index]));
			}
			//for(i=0;i<M;i++)
			//{
		...
		if ((selectedAction==ACTION_TERMINATE)||(i_iter<=CNBetaUpdateStart)||(i_iter%10==0))
		{
			ee					= 0;
			 //for(i=0;i<N;i++)
            //{
			...
			F77_CALL(daxpy)(&M, &a_blas,gamma, &inci,&temp, &inc0);//daxpy(n, a, x, incx, y, incy) y := a*x + y
			 ...
			 beta[0]			= (N-temp)/ee;
			 varT				= varTargetsGFNeg(Targets,N);
			// Rprintf("Gaussian update after: beta: %f\n",beta[0]);
			 ...
			 if (fabs(deltaLogBeta)>MinDeltaLogBeta)
			 {
				...
				FinalUpdateGFNeg(PHI,H,SIGMA,Targets,Mu,Alpha,beta,N, M);
...
void fEBInitializationGFNeg(double *Alpha, double *PHI, int *Used, int *Unused, double *BASIS,
			double *Targets, float *Scales, int * initial, int N, int *m, int K, double *beta)
{
    //basis dimension
    int M,M_full,i,j,kk;
  	kk					= K;
...
*/
			...
			F77_CALL(dcopy)(&N,readPtr1,&inci,PHI,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
			b_blas 			= 1/(double)Scales[loc1];
			F77_CALL(dscal)(&N,&b_blas,PHI,&inci); 		//dscal(n, a, x, incx) x = a*x

		// beta
		double stdT;
		stdT					= varTargetsGFNeg(Targets,N);
		stdT					= sqrt(stdT);
				...
				double *Targets, float *scales,int N,int K,int M,int M_full)
{
	...
	for (i						=0;i<K;i++)
	{
		for(l=0;l<M;l++)
		{
			...
			//BASIS_PHI[l*M_full+i]	= (float)(z2[l]/(double)scales[i]); //BASIS_PHI[L][I]
			BASIS_PHI[l][i]	= (float)(z2[l]/(double)scales[i]);
		}
		...
		BASIS_Targets[i]			= (float)(zTargets/(double)scales[i]);
		...
		if(i<(K-1))
		{
			for (j					= (i+1);j<K;j++)
			{
				for(h=0;h<M;h++)
				{
					...
					//BASIS_PHI[h*M_full+kk]	= (float)(z2[h]/(double)scales[kk]);//BASIS_PHI[H][KK]
					BASIS_PHI[h][kk]	= (float)(z2[h]/(double)scales[kk]);
				}

				zTargets			= 0;
				for(h=0;h<N;h++)	zTargets		= zTargets + BASIS[j*N+h]*cache1[h];
				BASIS_Targets[kk]	= (float)(zTargets/(double)scales[kk]);
				kk					= kk + 1;
			}
		}
	}
...
//H2 = t(PHI2)*PHI2*beta + diag(A)
	if(iteration[0]		== 1 && i_iter[0]==0)			// initialize SIGMA
	{
		...
		//save a copy of H
		SIGMA[0]		= 1/H[0];
		//MatrixInverseGFNeg(SIGMA,M);				//inverse of H2 is needed for wald score
	}
    ...
    for(i=0; i<M_full; i++)
    {
		...
		{
			BPvector[j]			= 0;
			//for(p=0;p<M;p++)	BPvector[j]		= BPvector[j] + (double)BASIS_PHI[p*M_full + i]*SIGMA[j*M+p]; //BASIS_PHI[P][I]
			for(p=0;p<M;p++)	BPvector[j]		= BPvector[j] + (double)BASIS_PHI[p][i]*SIGMA[j*M+p];
		}
		//readPtr1 	= &BASIS_PHI[i];
		//b_blas 		= 0;
        ...
        tempSum					= 0;
		//for(j=0;j<M;j++)		tempSum			= tempSum + BPvector[j]*(double)BASIS_PHI[j*M_full+i];//BASIS_PHI[J][I]
		for(j=0;j<M;j++)		tempSum			= tempSum + BPvector[j]*(double)BASIS_PHI[j][i];
		//readPtr1 	= &BASIS_PHI[i];
		//tempSum		= F77_CALL(ddot)(&M, BPvector, &inci,readPtr1, &M_full);

		S_in[i]					= (float)(beta[0] - beta[0]*tempSum*beta[0]);
		tempBPMu				= 0;
		//for(p=0;p<M;p++) tempBPMu = tempBPMu + (double)BASIS_PHI[p*M_full + i] *Mu[p];	//BASIS_PHI[P][I]
		for(p=0;p<M;p++) tempBPMu = tempBPMu + (double)BASIS_PHI[p][i] *Mu[p];
		//readPtr1 	= &BASIS_PHI[i];
		//tempBPMu 	= F77_CALL(ddot)(&M, Mu, &inci,readPtr1, &M_full);

		Q_in[i]					= (float)(beta[0]*((double)BASIS_Targets[i] - tempBPMu));
        S_out[i]         		= S_in[i];
        Q_out[i]          		= Q_in[i];
    }// Main loop ends
	//F77_CALL(dcopy)(&M_full,S_in,&inci,S_out,&incj);  //dcopy(n, x, incx, y, incy) ---> y = x
				...
				double *a_gamma, double *b_gamma, int M, int N_unused)
{
	...
	//Rprintf("Inside fEBDeltaMLGFNeg: a: %f, b %f \n",a, b);
    for(i=0;i<M;i++)
    {
        index						= Used[i] -1;
		DeltaML[index]				= 0;
        //    mexPrintf("N_used: \t %d,\t%d\n",N_used,index);
        alpha						= 2*a +2 + b*(double)S_out[index] - b*(double)Q_out[index]*(double)Q_out[index];
        beta						= (4*a + 5)*(double)S_out[index] + b*(double)S_out[index]*(double)S_out[index] -(double)Q_out[index]*(double)Q_out[index];
        gamma						= (2*a + 3)*(double)S_out[index]*(double)S_out[index];
        delta						= beta*beta - 4*alpha*gamma;
        //case1
        if(alpha>0 && delta>0 && beta<0)
        {
            root1					= (- beta-sqrt(delta))/(2*alpha);
            logL					= (log(root1/(root1+(double)S_out[index]))+pow((double)Q_out[index],2)/(root1+(double)S_out[index]))*0.5 + (a+1)*log(b*root1/(1+b*root1));
            if (logL > logL_inf)
            {
                ...
                //
                oldAlpha            = Alpha[i];
                DeltaML[index]  	= 0.5*(log(root1*(oldAlpha+(double)S_out[index])/(oldAlpha*(root1 + (double)S_out[index])))+
                                    (double)Q_out[index]*(double)Q_out[index]*(1/(root1+(double)S_out[index])-1/(oldAlpha+(double)S_out[index])))+
        ...
        else if(alpha <0)
        {
            ...
            //
            oldAlpha            = Alpha[i];
            DeltaML[index]  	= 0.5*(log(root2*(oldAlpha + (double)S_out[index])/(oldAlpha*(root2 + (double)S_out[index]))) +
                                    (double)Q_out[index]*(double)Q_out[index]*(1/(root2 + (double)S_out[index])-1/(oldAlpha + (double)S_out[index]))) +
        ...
        else if (M>1)
        {
            ...
            oldAlpha            = Alpha[i];
            logL                = (log(oldAlpha/(oldAlpha + (double)S_out[index]))+pow((double)Q_out[index],2)/(oldAlpha + (double)S_out[index]))*0.5 + (a+1)*log(b*oldAlpha/(1 + b*oldAlpha));
            DeltaML[index]      = - logL;
        }
    }
    //ADDITION
    for(i=0;i<N_unused;i++)
    {
        ...
        if(alpha>0 && delta>0 && beta<0)
        {
            ...
            if (logL > logL_inf)
            {
                ...
                anyToAdd            = 1;
                //
                DeltaML[index]  	= (log(root1/(root1+(double)S_out[index]))+pow((double)Q_out[index],2)/(root1+(double)S_out[index]))*0.5 + (a+1)*log(b*root1/(1+b*root1));
            }
        }
        //case 2
        else if(alpha==0 && beta<0)
        ...
        else if(alpha <0)
        {
            ...
            anyToAdd            = 1;
            //
            DeltaML[index]  	= (log(root2/(root2+(double)S_out[index]))+pow((double)Q_out[index],2)/(root2+(double)S_out[index]))*0.5 + (a+1)*log(b*root2/(1+b*root2));
        }
    }
			...
			float*Q_in, int nu, double*SIGMANEW, int M_full,int N, int K, int M)
{
	...
	float *BASIS_Phi		= (float *) R_Calloc(M_full,float);
	...
	//Rprintf("\t\t Inside ActionAddGFNeg: index: %d\n",index);
	for (i					= 0;i<K;i++)
	{
		BASIS_Phi[i]		= 0;
		for(h=0;h<N;h++)
		{
			...
			BASIS_Phi[i]	= BASIS_Phi[i] + (float)z[h];
		}
		BASIS_Phi[i]		= BASIS_Phi[i]/scales[i];
		BASIS_B_Phi[i]		= beta[0]*(double)BASIS_Phi[i];
		//
		if(i<K-1)
		{
			for (j					=(i+1);j<K;j++)
			{
				BASIS_Phi[kk]		= 0;
				for(h=0;h<N;h++)	BASIS_Phi[kk]	= BASIS_Phi[kk] + (float)(BASIS[j*N + h] *z[h]);
				BASIS_Phi[kk]		= BASIS_Phi[kk]/scales[kk];
				BASIS_B_Phi[kk]		= beta[0]*(double)BASIS_Phi[kk];
				kk					= kk +1;
			}
		}
	}
	...
	{
		temp				= 0;
		//for(j=0;j<M;j++)	temp				= temp + (double)BASIS_PHI[j*M_full + i]*tmpp[j];//BASIS_PHI[j][i]
		for(j=0;j<M;j++)	temp				= temp + (double)BASIS_PHI[j][i]*tmpp[j];
		//readPtr1 			= &BASIS_PHI[i];
		//temp 				= F77_CALL(ddot)(&M, readPtr1, &M_full,tmpp,&incj);

		mCi[i]				= BASIS_B_Phi[i] -  beta[0]*temp;
		//BASIS_PHI[M*M_full + i]					= (float)BASIS_Phi[i];//BASIS_PHI[M][i]
		//BASIS_PHI[M][i]					= (float)BASIS_Phi[i];
		S_in[i]				= S_in[i] - (float)(mCi[i]*mCi[i]*s_ii);
		Q_in[i]				= Q_in[i] - (float)(mu_i*mCi[i]);
	}
...
int ActionDelGFNeg(double*PHI, double*Alpha, double*SIGMA, double*SIGMANEW, float**BASIS_PHI,
		double*Mu, float*S_in, float*Q_in, double *beta, int jj, int N, int M, int M_full)
 {
	...
	{
		temp				= 0;
		//for(j=0;j<M;j++)	temp				= temp + (double)BASIS_PHI[j*M_full + i]*SIGMA[jj*M + j];//BASIS_PHI[j][i]
		for(j=0;j<M;j++)	temp				= temp + (double)BASIS_PHI[j][i]*SIGMA[jj*M + j];
		S_in[i]				= S_in[i] +  (float)(pow(beta[0]*temp,2)/SIGMA[jj*M + jj]);
		Q_in[i]				= Q_in[i] +  (float)(beta[0]*temp *Mujj /SIGMA[jj*M + jj]);
		//jPm = beta*temp
	}

	//BASIS_PHI(:,jj) = [];
	//for(i=0;i<M_full;i++) BASIS_PHI[jj*M_full + i]	= BASIS_PHI[index*M_full+i];//BASIS_PHI[jJ][i]
	//for(i=0;i<M_full;i++) BASIS_PHI[jj][i]	= BASIS_PHI[index][i];
	float *ptr;
	ptr=BASIS_PHI[jj];
...
double varTargetsGFNeg(double* Targets,int N)
{
	int i;
	double meanT			= 0;
	double stdT				= 0;
...
void FinalUpdateGFNeg(double *PHI, double *H,double *SIGMA, double *Targets,
